...
function renderTeamAnalysis(ids){
  ...
  teamAnalysis.querySelectorAll('[data-npc]').forEach(button=>button.onclick=()=>{
    ...
    const transitiveLines=row.transitive.map(t=>`<li>${displayFactionName(t.playerFaction)} allié à ${displayFactionName(t.allyFaction)} via ${t.treaty||'règle non résolue'} : le PNJ voit l’allié à ${t.observerAttitudeToAlly??'—'} (${t.observerAttitudeCategory}), poids transitive ${t.transitiveWeight??'—'}${t.networkCoefficient!==null&&t.networkCoefficient!==undefined?' · coefficient réseau '+t.networkCoefficientKey+' = '+t.networkCoefficient+(t.weightedNetworkCoefficient!==null?' · produit pondéré '+fmt(t.weightedNetworkCoefficient,2):''):''}. Contribution numérique finale inconnue du moteur natif. ${reproBadge(t.reproducibility)}</li>`).join('');
    const profile=row.strategicProfile;
    const personality=profile?`<li>Personnalité : ${profile.personalityKey||'—'} · composante stratégique ${profile.strategicComponent||'—'} · opportunisme ${profile.strategicComponentValues?.strategic_balance_opportunism_factor??'—'} · composante évaluation de guerre ${profile.dealEvaluationComponent||'—'}. ${reproBadge('exact-pre-game')}</li>`:'<li>Personnalité CAI non résolue.</li>';
    ...
    detail.innerHTML=`<h3>${displayFactionName(row.npcFaction)} — ${row.category.label}</h3>
      <h4>Exact avant partie ${reproBadge('exact-pre-game')}</h4><ul>${memberLines}${personality}${restrictions.map(r=>`<li>Diplomatie désactivée par ${r.campaignGroup} ↔ ${r.targetFactionSet}.</li>`).join('')}</ul>
