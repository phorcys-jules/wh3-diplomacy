...
def main():
    ...
    require(rows['start_factions'], {'faction', 'campaign', 'cai_starting_personality'}, TABLES['start_factions'])
    ...
    for row in rows['start_factions']:
        if row.get('campaign') != args.campaign or not row.get('faction'):
            continue
        personality_key = row.get('cai_starting_personality')
        personality = personalities.get(personality_key)
        profiles.append({
            'factionKey': row['faction'], 'personalityKey': personality_key or None,
    ...
    output = {
        ...
        'diagnostics': {'startingFactionCount': len(profiles), 'unresolvedPersonalityCount': sum(not item['personalityResolved'] for item in profiles)},
