...
def main():
    ...
    starts = table(args.db_dir / 'start_pos_factions_tables' / 'data__.tsv', {'faction', 'campaign', 'playable', 'is_major', 'faction_potential'})
    ...
    output = {'gameVersion': args.game_version, 'campaign': args.campaign, 'generatedAt': datetime.now(timezone.utc).isoformat(), 'status': 'partial', 'semantics': 'Every entry is explicitly active in the Immortal Empires start-position table. Missing name, leader or position values remain null rather than inferred.', 'factions': sorted(items, key=lambda item: item['factionKey']), 'diagnostics': {'activeFactionCount': len(items), 'playableFactionCount': sum(item['playable'] for item in items), 'positionedFactionCount': sum(item['position'] is not None for item in items), 'unresolvedSubcultureCount': sum(item['subculture'] is None for item in items)}}
